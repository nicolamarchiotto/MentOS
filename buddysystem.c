...
static unsigned long get_buddy_idx(unsigned long page_idx, unsigned int order)
{
	/*  Get the index of the buddy block.
		RITORNA INDICE PACINA BUDDY, SX O DX
     *
     *  ----------------------- xor -----------------------
...
block_found:
	...
	//ritorna la pagina che ha come campo lru &(area->free_list)
	//PRIMO PUNTATORE ALLA LISTA DI PAGINE LIBERE area->free.list.next;
	page = list_entry(area->free_list.next, page_t, lru);

	// Remove page from the list_head in the found free_area_t.
	//CONCATENA PAGINA PREC CON SUCC ELIMINANDOSI
	list_head_del(&(page->lru));

	// Set page as taken.
	//-1 libera, altrimenti occupata
	page->_count = 0;
	//dimensione =0
	page->private = 0;
	...
	//=2^current_order=pagine che ho preso
	...
	while (current_order > order) {
		...
		//DIMENSIONE BUDDY= DIMENSIONE PAGE -1
		current_order--;
		//SETTO DIMENSIONE BUDDY LIBERO
	...
	//restituisce al kernel per aggiornare...
